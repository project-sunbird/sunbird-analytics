...
echo "Running the $1 updater replay..." >> "logs/$end_date-$1-replay.log"
...
if [ $? == 0 ] 
	then
  	echo "$1 updater replay executed successfully..." >> "logs/$end_date-$1-replay.log"
else
 	echo "$1 updater replay failed" >> "logs/$end_date-$1-replay.log"
 	exit 1
fi