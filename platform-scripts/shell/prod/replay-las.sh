...
start_date=$1
end_date=$2
job_config='{"search":{"type":"s3","queries":[{"bucket":"prod-data-store","prefix":"ss/","endDate":"__endDate__","delta":6}]},"model":"org.ekstep.analytics.model.LearnerActivitySummary","output":[{"to":"console","params":{"printEvent":false}},{"to":"kafka","params":{"brokerList":"10.10.1.171:9092","topic":"prod.analytics.screener"}}],"parallelization":10,"appName":"TestReplaySupervisor","deviceMapping":false}'

./replay-backup $start_date $end_date "prod-data-store" "las" "backup-las"
if [ $? == 0 ]
 	then
...
if [ $? == 0 ] 
	then
  		...
  		./replay-delete "prod-data-store" "backup-las"
else
 	echo "Unable to take backup"
fi