...
if [ $? == 0 ]
 	then
  	...
  	$SPARK_HOME/bin/spark-submit --master local[*] --jars /mnt/data/analytics/models/analytics-framework-0.5.jar --class org.ekstep.analytics.job.ReplaySupervisor /mnt/data/analytics/models/batch-models-1.0.jar --model "$model_code" --fromDate "$start_date" --toDate "$end_date" --config "$job_config" > "logs/$end_date-lp.log"
else
