...
echo "Backing up the aser screener summary records to s3://sandbox-data-store/backup-as"
./replay-backup.sh $start_date $end_date "sandbox-data-store" "as" "backup-as"
if [ $? == 0 ]
 	then
