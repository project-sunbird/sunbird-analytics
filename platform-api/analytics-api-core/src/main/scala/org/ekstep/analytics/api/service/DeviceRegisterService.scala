...
class DeviceRegisterService(saveMetricsActor: ActorRef, config: Config) extends Actor {
    ...
    def generateDeviceRegistrationLogEvent(result: DeviceProfileLog): String = {
        ...
        val deviceProfile: Map[String, Any] =
          Map("device_id" -> s"${result.device_id.orElse("")}",
            "country_code" -> s"${result.location.countryCode.orElse("")}",
            "country" -> s"${result.location.countryName.orElse("")}",
            "state_code" -> s"${result.location.stateCode.orElse("")}",
            "state" -> s"${result.location.state.orElse("")}",
            "city" -> s"${result.location.city.orElse("")}",
            "state_custom" -> s"${result.location.stateCustom.orElse("")}",
            "state_code_custom" -> s"${result.location.stateCodeCustom.orElse("")}",
            "district_custom" -> s"${result.location.districtCustom.orElse("")}",
