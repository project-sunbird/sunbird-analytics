...
object ItemUsageMetricsModel extends IMetricsModel[ItemUsageSummaryView, ItemUsageMetrics]  with Serializable {
  	...
  	private def reduceItemSummary(fact1: ItemUsageSummary, fact2: ItemUsageSummary) : ItemUsageSummary = {
		...
		val m_top5_incorrect_res = (fact1.m_top5_incorrect_res.getOrElse(List()) ++ fact2.m_top5_incorrect_res.getOrElse(List())).groupBy(f => f._1).mapValues(f => f.map(x => x._2).sum).toList
									.sorted(Ordering.by((_: (String, Int))._2).reverse).take(5).map { x => x }.toList;
