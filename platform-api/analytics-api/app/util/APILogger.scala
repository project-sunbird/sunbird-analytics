...
object APILogger {
  def init(jobName: String) = {
    val apiConf = play.Play.application.configuration.underlying().withFallback(ConfigFactory.parseMap(Map( "log4j.appender.kafka.broker_host" -> "", "log4j.appender.kafka" -> "", "log4j.appender.kafka.topic" -> "").asJava ))
    val ctx = LogManager.getContext(false).asInstanceOf[LoggerContext];
    ctx.reconfigure();
    if (StringUtils.equalsIgnoreCase(apiConf.getString("log4j.appender.kafka"), "true")) {
