...
class ApplicationSpec extends Specification {
    ...
    "Application" should {
       ...
       "invoke run job" in new WithApplication {
			val req = Json.toJson(Json.parse(""" {} """))
			val home = route(FakeRequest(POST, "/dataproduct/run/ss", FakeHeaders(Seq(("content-type", "application/json"))), req)).get
			status(home) must equalTo(OK)
			contentType(home) must beSome.which(_ == "application/json")
       }
       ...
       "invoke replay-job" in new WithApplication {
			val req = Json.toJson(Json.parse(""" {} """))
			val home = route(FakeRequest(POST, "/dataproduct/replay/ss/2016-04-01/2016-04-03", FakeHeaders(Seq(("content-type", "application/json"))), req)).get
			status(home) must equalTo(OK)
			contentType(home) must beSome.which(_ == "application/json")
       }
