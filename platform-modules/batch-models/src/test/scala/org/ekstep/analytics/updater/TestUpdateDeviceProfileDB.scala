...
class TestUpdateDeviceProfileDB extends SparkSpec(null) {
    ...
    "UpdateDeviceProfileDB" should "create device profile in device db" in {
        ...
        val device1 = sc.cassandraTable[DeviceProfileOutput](Constants.DEVICE_KEY_SPACE_NAME, Constants.DEVICE_PROFILE_TABLE).where("device_id=?", "88edda82418a1e916e9906a2fd7942cb").where("channel=?", "b00bc992ef25f1a9a8d63291e20efc8d").first
    ...
    it should "check for first_access and last_access" in {
        ...
        val device1 = sc.cassandraTable[DeviceProfileOutput](Constants.DEVICE_KEY_SPACE_NAME, Constants.DEVICE_PROFILE_TABLE).where("device_id=?", "48edda82418a1e916e9906a2fd7942cb").where("channel=?", "b00bc992ef25f1a9a8d63291e20efc8d").first
        device1.first_access.get should be(1537550355883L)
