...
class TestContentLanguageRelationModel extends SparkGraphSpec(null) {
    ...
    it should "create Language nodes and 'expressedIn' relation with contents and pass the test cases" in {
        ...
        val langNodesBefore = GraphQueryDispatcher.dispatch(findLanguageNodesQuery).list();
        langNodesBefore.size() should be(3)

        val contentLangRelBefore = GraphQueryDispatcher.dispatch(contentLangRelationsQuery).list;
        ...
        val langNodesAfter = GraphQueryDispatcher.dispatch(findLanguageNodesQuery).list();
        ...
        val eng = GraphQueryDispatcher.dispatch(queryForEng).list();
        ...
        val other = GraphQueryDispatcher.dispatch(queryForOther).list();
        other.get(0).get("l").asMap().get("contentCount") should be(1)
        other.get(0).get("l").asMap().get("liveContentCount") should be(0)

        val contentLangRelAfter = GraphQueryDispatcher.dispatch(contentLangRelationsQuery).list;
        ...
        val languageContentRels = GraphQueryDispatcher.dispatch(languageContentRelQuery).list;
        ...
        val languageLanguageRels = GraphQueryDispatcher.dispatch(languageLanguageRelQuery).list;
