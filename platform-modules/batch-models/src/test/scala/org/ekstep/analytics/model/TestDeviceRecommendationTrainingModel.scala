...
class TestDeviceRecommendationTrainingModel extends SparkSpec(null) {
    ...
    "DeviceRecommendationTrainingModel" should "generate libsvm files and save model to s3" in {
        ...
        val me = DeviceRecommendationTrainingModel.execute(null, Option(Map("trainRatio" -> Double.box(0.5), "testRatio" -> Double.box(0.5), "libfm.executable_path" -> "src/test/resources/device-reco/")))
        val res = me.collect()
