...
class TestDeviceRecommendationScoringModel extends SparkSpec(null) {
    ...
    it should "load model with zero W0, generate scores and check for filter contents already in the device" in {

        REScoringMock.execute(null, Option(Map("model_name" -> "fm.model3", "localPath" -> "src/test/resources/device-recos-training/RE-data/", "dataTimeFolderStructure" -> false.asInstanceOf[AnyRef], "key" -> "model/test/", "filterBlacklistedContents" -> false.asInstanceOf[AnyRef])))
        CommonUtil.deleteDirectory("src/test/resources/device-recos-training/RE-data");
    }

    it should "load model with zero unary interactions, generate scores" in {

        REScoringMock.execute(null, Option(Map("model_name" -> "fm.model4", "localPath" -> "src/test/resources/device-recos-training/RE-data/", "dataTimeFolderStructure" -> false.asInstanceOf[AnyRef], "key" -> "model/test/", "filterBlacklistedContents" -> true.asInstanceOf[AnyRef])))
        CommonUtil.deleteDirectory("src/test/resources/device-recos-training/RE-data");
    }
