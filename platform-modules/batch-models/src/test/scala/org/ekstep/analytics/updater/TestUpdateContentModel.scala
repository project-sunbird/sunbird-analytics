...
class TestUpdateContentModel extends SparkSpec(null) {
    ...
    it should "populate content usage metrics when popularity metrics are blank in content model and vice-versa" in {
        ...
        val workflowUsageSummaries = Array(WorkFlowUsageSummaryFact(0, AppConf.getConfig("default.channel.id"), AppConf.getConfig("default.app.id"), "all", "Textbook", "mode1", "dId-1", "org.ekstep.delta", "userId-1", DateTime.now, DateTime.now, DateTime.now, 450.0, 4, 112.5, 100, 23.56, 11, 2.15, 12, 15, 18, Array(1),Array(2),Array(3), "Textbook"),
            WorkFlowUsageSummaryFact(0, AppConf.getConfig("default.channel.id"), AppConf.getConfig("default.app.id"), "all", "Worksheet", "mode1", "dId-2", "numeracy_374", "userId-2", DateTime.now, DateTime.now, DateTime.now, 220.5, 4, 52.5, 76, 23.56, 11, 2.15, 12, 15, 18, Array(1),Array(2),Array(3), "Worksheet"));
        sc.parallelize(workflowUsageSummaries).saveToCassandra(Constants.PLATFORM_KEY_SPACE_NAME, Constants.WORKFLOW_USAGE_SUMMARY_FACT);
    ...
    it should "return zero output when no records found for given date" in {
        ...
        val workflowUsageSummaries = Array(WorkFlowUsageSummaryFact(0, AppConf.getConfig("default.channel.id"), AppConf.getConfig("default.app.id"), "all", "Textbook", "mode1", "dId-1", "org.ekstep.delta", "userId-1", DateTime.now, DateTime.now, DateTime.now, 450.0, 4, 112.5, 100, 23.56, 11, 2.15, 12, 15, 18, Array(1),Array(2),Array(3), "Textbook", Option(DateTime.now().minusDays(2))),
            WorkFlowUsageSummaryFact(0, AppConf.getConfig("default.channel.id"), AppConf.getConfig("default.app.id"), "all", "Worksheet", "mode1", "dId-2", "numeracy_374", "userId-2", DateTime.now, DateTime.now, DateTime.now, 220.5, 4, 52.5, 76, 23.56, 11, 2.15, 12, 15, 18, Array(1),Array(2),Array(3), "Worksheet", Option(DateTime.now().minusDays(2))));
        sc.parallelize(workflowUsageSummaries).saveToCassandra(Constants.PLATFORM_KEY_SPACE_NAME, Constants.WORKFLOW_USAGE_SUMMARY_FACT);
    ...
    it should "not include workflow summary records having content_id 'all'" in {
        ...
        val workflowUsageSummaries = Array(WorkFlowUsageSummaryFact(0, AppConf.getConfig("default.channel.id"), AppConf.getConfig("default.app.id"), "all", "Textbook", "mode1", "dId-1", "all", "userId-1", DateTime.now, DateTime.now, DateTime.now, 450.0, 4, 112.5, 100, 23.56, 11, 2.15, 12, 15, 18, Array(1),Array(2),Array(3), "Textbook"),
            WorkFlowUsageSummaryFact(0, AppConf.getConfig("default.channel.id"), AppConf.getConfig("default.app.id"), "all", "Worksheet", "mode1", "dId-2", "numeracy_374", "userId-2", DateTime.now, DateTime.now, DateTime.now, 220.5, 4, 52.5, 76, 23.56, 11, 2.15, 12, 15, 18, Array(1),Array(2),Array(3), "Worksheet"));
        sc.parallelize(workflowUsageSummaries).saveToCassandra(Constants.PLATFORM_KEY_SPACE_NAME, Constants.WORKFLOW_USAGE_SUMMARY_FACT);
