...
class TestCreationRecommendationEnrichmentModel extends SparkGraphSpec(null) {
    ...
    it should "set confidence & lift value in the relations between Author -> Concept, Author -> Language & Author -> ContentType" in {
        ...
        val usesRels1 = GraphQueryDispatcher.dispatch(graphConfig, "MATCH (usr:User{type: 'author'})-[r:uses]-(cnc:domain{IL_FUNC_OBJECT_TYPE:'Concept'}) WHERE cnc.contentCount > 0 AND usr.IL_UNIQUE_ID = '290' RETURN r").list
        ...
        val usesRels2 = GraphQueryDispatcher.dispatch(graphConfig, "MATCH (usr:User{type: 'author'})-[r:uses]-(cnc:domain{IL_FUNC_OBJECT_TYPE:'Concept'}) WHERE cnc.contentCount > 0 AND usr.IL_UNIQUE_ID = '291' RETURN r").list
        ...
        val createdInRels1 = GraphQueryDispatcher.dispatch(graphConfig, "MATCH (usr:User{type:'author'})-[r:createdIn]->(lan:domain{IL_FUNC_OBJECT_TYPE:'Language'}) WHERE lan.contentCount > 0 AND usr.IL_UNIQUE_ID = '290' AND lan.IL_UNIQUE_ID = 'lang_en' RETURN r").list
        ...
        val createdInRels2 = GraphQueryDispatcher.dispatch(graphConfig, "MATCH (usr:User{type:'author'})-[r:createdIn]->(lan:domain{IL_FUNC_OBJECT_TYPE:'Language'}) WHERE lan.contentCount > 0 AND usr.IL_UNIQUE_ID = '291' AND lan.IL_UNIQUE_ID = 'lang_en' RETURN r").list
        ...
        val usesRels3 = GraphQueryDispatcher.dispatch(graphConfig, "MATCH (usr:User{type:'author'})-[r:uses]->(cntt:ContentType{ IL_UNIQUE_ID: 'story'}) WHERE cntt.contentCount > 0 AND usr.IL_UNIQUE_ID = '290' RETURN r").list
        ...
        val usesRels4 = GraphQueryDispatcher.dispatch(graphConfig, "MATCH (usr:User{type:'author'})-[r:uses]->(cntt:ContentType{ IL_UNIQUE_ID: 'story'}) WHERE cntt.contentCount > 0 AND usr.IL_UNIQUE_ID = '291' RETURN r").list
