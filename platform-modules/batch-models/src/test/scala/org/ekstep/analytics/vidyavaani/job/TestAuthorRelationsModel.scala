...
class TestAuthorRelationsModel extends SparkGraphSpec(null) {
    ...
    it should "create Author nodes and its relation with contents, concepts & languages" in {
        ...
        val contentNodes = GraphQueryDispatcher.dispatch(findContentNodesQuery).list;
        contentNodes.size() should be(4)

        GraphQueryDispatcher.dispatch(deleteAuthorNodesQuery)

        // Check for User node and its relations before running AuthorRelationsModel
        val authorNodesBefore = GraphQueryDispatcher.dispatch(getAuthorNodesQuery).list;
        authorNodesBefore.size() should be(0)

        val contentAuthorRelBefore = GraphQueryDispatcher.dispatch(getContentAuthorRelQuery).list;
        contentAuthorRelBefore.size should be(0)
        
        val conceptAuthorRelBefore = GraphQueryDispatcher.dispatch(getConceptAuthorRelQuery).list;
        conceptAuthorRelBefore.size should be(0)
        
        val langAuthorRelBefore = GraphQueryDispatcher.dispatch(getLanguageAuthorRelQuery).list;
        ...
        val authorNodesAfter = GraphQueryDispatcher.dispatch(getAuthorNodesQuery).list;
        authorNodesAfter.size() should be(2)
        
        // Check for contentCount, liveContentCount & conceptsCount on User node
        val author1 = GraphQueryDispatcher.dispatch("MATCH (usr:User {type: 'author'}) WHERE usr.IL_UNIQUE_ID = '290' RETURN usr").list;
        ...
        val author2 = GraphQueryDispatcher.dispatch("MATCH (usr:User {type: 'author'}) WHERE usr.IL_UNIQUE_ID = '291' RETURN usr").list;
        ...
        val usesRel1 = GraphQueryDispatcher.dispatch("MATCH (cnt: domain {IL_FUNC_OBJECT_TYPE: 'Concept'} ) <-[r:uses]- (usr: User {type: 'author'}) WHERE usr.IL_UNIQUE_ID = '290' RETURN r").list;
        ...
        val usesRel2 = GraphQueryDispatcher.dispatch("MATCH (cnt: domain {IL_FUNC_OBJECT_TYPE: 'Concept'} ) <-[r:uses]- (usr: User {type: 'author'}) WHERE usr.IL_UNIQUE_ID = '291' RETURN r").list;
        ...
        val langAuthorRelAfter = GraphQueryDispatcher.dispatch(getLanguageAuthorRelQuery).list;
        langAuthorRelAfter.size should be(6)

        // Check for contentCount & liveContentCount on User `createdIn` Language relationship
        val createdInRel1 = GraphQueryDispatcher.dispatch("MATCH (usr:User {type:'author'})-[r:createdIn]->(lan:domain{IL_FUNC_OBJECT_TYPE:'Language'}) WHERE usr.IL_UNIQUE_ID = '290' AND lan.IL_UNIQUE_ID = 'lang_en' RETURN r").list;
        ...
        val createdInRel2 = GraphQueryDispatcher.dispatch("MATCH (usr:User {type:'author'})-[r:createdIn]->(lan:domain{IL_FUNC_OBJECT_TYPE:'Language'}) WHERE usr.IL_UNIQUE_ID = '291' AND lan.IL_UNIQUE_ID = 'lang_en' RETURN r").list;
        ...
        val authorContentRels = GraphQueryDispatcher.dispatch(authorContentRelQuery).list;
        authorContentRels.size should be(0)

        val authorAuthorRelQuery = "MATCH (usr1: User) -[r:createdBy]-> (usr2: User) RETURN r"
        val authorOwnerRels = GraphQueryDispatcher.dispatch(authorAuthorRelQuery).list;
        authorOwnerRels.size should be(0)

        val contentContentRelQuery = "MATCH (cnt1: domain) -[r:createdBy]-> (cnt2: domain) RETURN r"
        val contentContentRels = GraphQueryDispatcher.dispatch(contentContentRelQuery).list;
    ...
    it should "create ContentType nodes and its relation with Authors & Contents" in {
        ...
        GraphQueryDispatcher.dispatch(deleteContentTypeNodes)
        
        // Check for ContentType node and its relations before running AuthorRelationsModel
        val contentTypeNodesBefore = GraphQueryDispatcher.dispatch(getContentTypeNodesQuery).list;
        contentTypeNodesBefore.size() should be(0)

        val contentTypeAuthorRelBefore = GraphQueryDispatcher.dispatch(getContentTypeAuthorRelQuery).list;
        contentTypeAuthorRelBefore.size should be(0)
        
        val contentContentTypeRelBefore = GraphQueryDispatcher.dispatch(getContentContentTypeRelQuery).list;
        ...
        val contentTypeNodesAfter = GraphQueryDispatcher.dispatch(getContentTypeNodesQuery).list;
        contentTypeNodesAfter.size() should be(4)
        
        // Check for contentCount & liveContentCount on ContentType node
        val contentType1 = GraphQueryDispatcher.dispatch("MATCH (cntt: ContentType{ IL_UNIQUE_ID: 'story'}) RETURN cntt").list;
        ...
        val contentType2 = GraphQueryDispatcher.dispatch("MATCH (cntt: ContentType{ IL_UNIQUE_ID: 'worksheet'}) RETURN cntt").list;
        ...
        val contentContentTypeRelAfter = GraphQueryDispatcher.dispatch(getContentContentTypeRelQuery).list;
        contentContentTypeRelAfter.size should be(4)
        
        val contentTypeAuthorRelAfter = GraphQueryDispatcher.dispatch(getContentTypeAuthorRelQuery).list;
        contentTypeAuthorRelAfter.size should be(8)

        // Check for contentCount & liveContentCount on User `uses` ContentType relationship
        val usesRel1 = GraphQueryDispatcher.dispatch("MATCH (usr:User {type:'author'})-[r:uses]->(cntt:ContentType) WHERE usr.IL_UNIQUE_ID = '290' AND cntt.IL_UNIQUE_ID = 'story' RETURN r").list;
        usesRel1.size() should be(1)
        usesRel1.get(0).get("r").asMap().get("contentCount") should be(3)
        usesRel1.get(0).get("r").asMap().get("liveContentCount") should be(1)
        
        val usesRel2 = GraphQueryDispatcher.dispatch("MATCH (usr:User {type:'author'})-[r:uses]->(cntt:ContentType) WHERE usr.IL_UNIQUE_ID = '291' AND cntt.IL_UNIQUE_ID = 'story' RETURN r").list;
        usesRel2.size() should be(1)
        usesRel2.get(0).get("r").asMap().get("contentCount") should be(0)
        usesRel2.get(0).get("r").asMap().get("liveContentCount") should be(0)
