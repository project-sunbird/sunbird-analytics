...
//        
//        val ownerNodesBefore = GraphDBUtil.findNodes(Map(), Option(List(AuthorRelationsModel.NODE_NAME)));
...
//
//        val ownerNodesAfter = GraphDBUtil.findNodes(Map(), Option(List(AuthorRelationsModel.NODE_NAME)));
...
//        .append(GraphDBUtil.getRelationQuery(AuthorRelationsModel.RELATION, RelationshipDirection.OUTGOING.toString)).append(BLANK_SPACE)
