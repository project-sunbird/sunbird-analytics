...
class TestUpdateContentRating  extends SparkSpec(null) with MockFactory {
    ...
    "UpdateContentRating" should "get content list which are rated in given time" in {
        ...
        (mockRestUtil.post[List[Map[String, AnyRef]]](_: String, _: String, _: Option[Map[String, String]])(_: Manifest[List[Map[String, AnyRef]]])).expects("http://localhost:8082/druid/v2/sql/", "{\"query\":\"SELECT DISTINCT \\\"object_id\\\" AS \\\"Id\\\"\\nFROM \\\"druid\\\".\\\"%s\\\" WHERE \\\"eid\\\" = 'FEEDBACK' AND \\\"syncts\\\" BETWEEN %d AND %d \"}".format("telemetry-events", new DateTime(startDate).getMillis, new DateTime(endDate).getMillis), None, manifest[List[Map[String, AnyRef]]])
    ...
    it should "get all content ratings" in {
        ...
        val mockRestUtil = mock[HTTPClient]
        (mockRestUtil.post[List[Map[String, AnyRef]]](_: String, _: String, _: Option[Map[String, String]])(_: Manifest[List[Map[String, AnyRef]]])).expects("http://localhost:8082/druid/v2/sql/", "{\"query\": \"SELECT \\\"object_id\\\" AS ContentId, COUNT(*) AS \\\"Number of Ratings\\\", SUM(edata_rating) AS \\\"Total Ratings\\\", SUM(edata_rating)/COUNT(*) AS \\\"AverageRating\\\" FROM \\\"druid\\\".\\\"telemetry-events\\\" WHERE \\\"eid\\\" = 'FEEDBACK' GROUP BY \\\"object_id\\\"\"}", None, manifest[List[Map[String, AnyRef]]])
    ...
    it should "check for system update API call" in {
        ...
        val mockResponse =
          s"""
             ...
             |}
           """.stripMargin
        (mockRestUtil.post[Response](_: String, _: String, _: Option[Map[String, String]])(_: Manifest[Response])).expects(systemUpdateURL, request, None, manifest[Response])
          .returns(JSONUtils.deserialize[Response](mockResponse))

        val response = UpdateContentRating.publishRatingToContentModel("test-1", 5, "http://localhost:8080/learning-service/system/v3/content/update", mockRestUtil)
