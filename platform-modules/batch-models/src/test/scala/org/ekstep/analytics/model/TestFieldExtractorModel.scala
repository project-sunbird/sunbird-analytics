...
class TestFieldExtractorModel extends SparkSpec(null) {
    ...
    "FieldExtractorModel" should "extract the fields from raw telemetry" in {
        ...
        val rdd2 = EventFieldExtractorModel.execute(rdd, Option(Map("headers" -> "eid,ts,timespent,gameId", "fields" -> "eid,ts,edata.eks.length,gdata.id")));
    ...
    it should "extract the fields from derived telemetry" in {
        ...
        val rdd = DerivedEventFieldExtractorModel.execute(events, Option(Map("headers" -> "eid,ts,timespent,gameId,groupuser", "fields" -> "eid,ets,edata.eks.timeSpent,dimensions.gdata.id,dimensions.group_user")));
