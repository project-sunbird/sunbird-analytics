...
class TestContentSummary extends SparkSpec(null) {
    ...
    "ContentSummary" should "generate contentsummary and pass all positive test cases" in {
        ...
        val rdd = loadFile[MEEvent]("src/test/resources/CAS/test_data_1.log");
        ...
        val rdd2 = loadFile[MEEvent]("src/test/resources/CAS/test_data_2.log");
        val me2 = CAS.execute(rdd2, None).collect();
        val event2 = CommonUtil.getMeasuredEvent(me2(0));
        ...
        val rdd3 = loadFile[MEEvent]("src/test/resources/CAS/test_data_3.log");
        val me3 = CAS.execute(rdd3, None).collect();
        val event3 = CommonUtil.getMeasuredEvent(me3(0));
    ...
    it should "generate contentsummary for more than 5 content, those are not Collection type" in {
        val rdd = loadFile[MEEvent]("src/test/resources/CAS/test_data_4.log");
        val me = CAS.execute(rdd, None).collect;
    }
}