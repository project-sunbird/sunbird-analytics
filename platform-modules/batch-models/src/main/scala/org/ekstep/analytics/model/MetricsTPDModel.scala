...
object MetricsTPDModel extends IBatchModelTemplate[Empty, Empty, CourseEnrollmentOutput, CourseEnrollmentOutput] with Serializable {

  implicit val className: String = "org.ekstep.analytics.model.MetricsTPDModel"
  ...
  override def algorithm(events: RDD[Empty], config: Map[String, AnyRef])(implicit sc: SparkContext, fc: FrameworkContext): RDD[CourseEnrollmentOutput] = {
    ...
    finalRDD.map(c=> CourseEnrollmentOutput("date",c._2._1._2.get.courseName,c._2._1._1.name,c._2._1._1.status,c._2._1._1.participantCount,c._2._1._1.completedCount,c._2._2.get.slug,"course_enrollment"))
  ...
  def getLiveCoursesFromDruid()(implicit sc: SparkContext): RDD[CourseInfo] = {
    ...
    val druidResponse = DruidDataFetcher.getDruidData(query)
    val response = druidResponse.map{x => JSONUtils.deserialize[Map[String, String]](x)}
    val liveCourses = response.map(c=> CourseInfo("channel-01", c.get("identifier").get, c.get("name").get, c.get("total_live_courses").get.toDouble))
