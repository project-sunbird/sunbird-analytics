 ...
 */
case class PortalUsageSummaryIndex(d_period: Int, d_author_id: String, d_app_id: String, d_channel_id: String) extends Output
case class PortalUsageSummaryFact(d_period: Int, d_author_id: String, d_app_id: String, d_channel_id: String, anon_total_sessions: Long, anon_total_ts: Double,
...
case class PortalUsageSummaryFact_T(d_period: Int, d_author_id: String, d_app_id: String, d_channel_id: String, last_gen_date: Long, anon_total_sessions: Long, anon_total_ts: Double,
                                    total_sessions: Long, total_ts: Double, ce_total_sessions: Long, ce_percent_sessions: Double,
...
object UpdateAppUsageDB extends IBatchModelTemplate[DerivedEvent, DerivedEvent, PortalUsageSummaryFact, PortalUsageSummaryIndex] with IInfluxDBUpdater with Serializable {
    ...
    override def algorithm(data: RDD[DerivedEvent], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[PortalUsageSummaryFact] = {
        ...
        val portalSummary = data.map { x =>
            ...
            PortalUsageSummaryFact_T(period, authorId, appId, channelId, x.context.date_range.to, anon_total_sessions, anon_total_ts, total_sessions, total_ts, ce_total_sessions, ce_percent_sessions,
    ...
    override def postProcess(data: RDD[PortalUsageSummaryFact], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[PortalUsageSummaryIndex] = {
        ...
        saveToInfluxDB(data);
        data.map { x => PortalUsageSummaryIndex(x.d_period, x.d_author_id, x.d_app_id, x.d_channel_id) };
    }
    ...
    private def rollup(data: RDD[PortalUsageSummaryFact_T], period: Period): RDD[PortalUsageSummaryFact] = {
        ...
        val currentData = data.map { x =>
            ...
            (PortalUsageSummaryIndex(d_period, x.d_author_id, x.d_app_id, x.d_channel_id), PortalUsageSummaryFact_T(d_period, x.d_author_id, x.d_app_id, x.d_channel_id, x.last_gen_date, x.anon_total_sessions, x.anon_total_ts, x.total_sessions, x.total_ts, x.ce_total_sessions, x.ce_percent_sessions,
        ...
        val prvData = currentData.map { x => x._1 }.joinWithCassandraTable[PortalUsageSummaryFact](Constants.CREATION_METRICS_KEY_SPACE_NAME, Constants.APP_USAGE_SUMMARY_FACT).on(SomeColumns("d_period", "d_author_id", "d_app_id", "d_channel_id"));
        val joinedData = currentData.leftOuterJoin(prvData)
        val rollupSummaries = joinedData.map { x =>
            val index = x._1
            val newSumm = x._2._1
            val prvSumm = x._2._2.getOrElse(PortalUsageSummaryFact(index.d_period, index.d_author_id, index.d_app_id, index.d_channel_id, 0L, 0.0, 0L, 0.0, 0L, 0.0, 0L, BloomFilterUtil.getDefaultBytes(period), 0L, 0.0, 0.0, 0.0, 0L, 0.0, 0L));
    ...
    private def reducePUS(fact1: PortalUsageSummaryFact_T, fact2: PortalUsageSummaryFact_T): PortalUsageSummaryFact_T = {
        ...
        PortalUsageSummaryFact_T(fact1.d_period, fact1.d_author_id, fact1.d_app_id, fact1.d_channel_id, fact1.last_gen_date, anon_total_sessions, anon_total_ts, total_sessions, total_ts, ce_total_sessions, ce_percent_sessions,
            total_pageviews_count, unique_users, unique_users_count, avg_pageviews, avg_ts_session, anon_avg_ts_session, new_user_count, percent_new_users_count);
    }
    ...
    private def reduce(fact1: PortalUsageSummaryFact, fact2: PortalUsageSummaryFact_T, period: Period): PortalUsageSummaryFact = {
        ...
        PortalUsageSummaryFact(fact1.d_period, fact1.d_author_id, fact1.d_app_id, fact1.d_channel_id, anon_total_sessions, anon_total_ts, total_sessions, total_ts, ce_total_sessions, ce_percent_sessions,
    ...
    private def saveToInfluxDB(data: RDD[PortalUsageSummaryFact])(implicit sc: SparkContext) {
        val metrics = data.filter { x => x.d_period != 0 }.map { x =>
            val fields = (CommonUtil.caseClassToMap(x) - ("d_period", "d_author_id", "d_app_id", "d_channel_id", "unique_users", "updated_date")).map(f => (f._1, f._2.asInstanceOf[Number].doubleValue().asInstanceOf[AnyRef]));
            val time = getDateTime(x.d_period);
            InfluxRecord(Map("period" -> time._2, "author_id" -> x.d_author_id, "app_id" -> x.d_app_id, "channel_id" -> x.d_channel_id), fields, time._1);
        };
