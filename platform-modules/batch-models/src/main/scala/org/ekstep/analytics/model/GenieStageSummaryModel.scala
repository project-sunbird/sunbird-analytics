...
case class GenieStageAlgoOut(stageId: String, sid: String, timeSpent: Double, visitCount: Int, interactEventsCount: Int, interactEvents: List[Map[String, String]], dt_range: DtRange, did: String, tags: Option[AnyRef], syncts: Long, pdata: PData, channelId: String) extends AlgoOutput
...
object GenieStageSummaryModel extends SessionBatchModel[DerivedEvent, MeasuredEvent] with IBatchModelTemplate[DerivedEvent, DerivedEvent, GenieStageAlgoOut, MeasuredEvent] with Serializable {
        ...
        data.map { event =>
            ...
            if (null != screenSummaries && screenSummaries.size > 0) {
                screenSummaries.map { x =>
                    ...
                    GenieStageAlgoOut(ss.stageId, ss.sid, ss.timeSpent, ss.visitCount, ss.interactEventsCount, ss.interactEvents, event.context.date_range, did, Option(tags), event.syncts, pdata, channelId)
                }
            } else {
    ...
    override def postProcess(data: RDD[GenieStageAlgoOut], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
        ...
        data.map { summary =>
            val mid = CommonUtil.getMessageId("ME_GENIE_STAGE_SUMMARY", summary.stageId + summary.sid, config.getOrElse("granularity", "GENIE SESSION").asInstanceOf[String], summary.dt_range.to, Option(summary.pdata.id), Option(summary.channelId));
            val measures = Map(
            ...
            MeasuredEvent("ME_GENIE_STAGE_SUMMARY", System.currentTimeMillis(), summary.syncts, "1.0", mid, null, Option(summary.channelId), None, None,
