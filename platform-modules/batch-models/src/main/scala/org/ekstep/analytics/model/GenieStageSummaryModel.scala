...
case class GenieStageAlgoOut(stageId: String, sid: String, timeSpent: Double, visitCount: Int, interactEventsCount: Int, interactEvents: List[Map[String, String]], dt_range: DtRange, syncts: Long) extends AlgoOutput
        ...
        data.map { event =>
            ...
            if (null != screenSummaries && screenSummaries.size > 0) {
                screenSummaries.map { x =>
                    ...
                    GenieStageAlgoOut(ss.stageId, ss.sid, ss.timeSpent, ss.visitCount, ss.interactEventsCount, ss.interactEvents, event.context.date_range, event.syncts)
                }
            } else {
    ...
    override def postProcess(data: RDD[GenieStageAlgoOut], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
        ...
        data.map { summary =>
            ...
            MeasuredEvent("ME_GENIE_STAGE_SUMMARY", System.currentTimeMillis(), summary.syncts, "1.0", mid, null, None, None,
                ...
                Dimensions(None, None, None, None, None, None, None, None, None, None, None, None, None, None, Option(summary.sid), Option(summary.stageId)), MEEdata(measures), None);
        };
    }
}