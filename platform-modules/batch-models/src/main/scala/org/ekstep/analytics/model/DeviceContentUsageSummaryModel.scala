...
case class DeviceContentSummary(device_id: String, content_id: String, app_id: String, channel: String, game_ver: Option[String], num_sessions: Option[Long], total_interactions: Option[Long], avg_interactions_min: Option[Double],
...
case class DeviceContentSummaryIndex(device_id: String, content_id: String, app_id: String, channel: String)
case class DeviceSummaryIndex(device_id: String, app_id: String, channel: String)
...
object DeviceContentUsageSummaryModel extends IBatchModelTemplate[DerivedEvent, DeviceSummaryInput, DeviceContentSummary, DeviceContentSummary] with Serializable {
    ...
    override def preProcess(data: RDD[DerivedEvent], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[DeviceSummaryInput] = {
        ...
        val deviceSessions = filteredEvents.map { event =>
            ...
            (DeviceSummaryIndex(did, pdata.id, channel), Buffer(event));
    ...
    override def algorithm(data: RDD[DeviceSummaryInput], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[DeviceContentSummary] = {
        ...
        val deviceDetails = data.map { deviceSummary =>
            val prevSummary = deviceSummary.prevData.getOrElse(DeviceUsageSummary(deviceSummary.index.device_id, deviceSummary.index.app_id, deviceSummary.index.channel, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None));
            ...
            (prevSummary.device_id, prevSummary.app_id, prevSummary.channel, Option(play_start_time), Option(last_played_on), Option(total_play_time), Option(num_sessions), Option(mean_play_time), Option(mean_play_time_interval), Option(last_played_content))
        }
        deviceDetails.saveToCassandra(Constants.DEVICE_KEY_SPACE_NAME, Constants.DEVICE_USAGE_SUMMARY_TABLE, SomeColumns("device_id", "app_id", "channel","play_start_time","last_played_on","total_play_time","num_sessions","mean_play_time","mean_play_time_interval","last_played_content"))
        ...
        val dcuSummaries = inputEvents.map { event =>
            ...
            val pdata = CommonUtil.getAppDetails(event)
            val channel = CommonUtil.getChannelId(event)
            (DeviceContentSummaryIndex(did, content_id, pdata.id, channel), Buffer(event));
        }.partitionBy(new HashPartitioner(JobContext.parallelization)).reduceByKey((a, b) => a ++ b);
        val prevDeviceContentSummary = dcuSummaries.map(f => f._1).joinWithCassandraTable[DeviceContentSummary](Constants.DEVICE_KEY_SPACE_NAME, Constants.DEVICE_CONTENT_SUMMARY_FACT).on(SomeColumns("device_id", "content_id", "app_id", "channel")) //.map(f => ((f._1.device_id, f._1.content_id), f._2))
        ...
        dcusEvents.map { dcusEvent =>
            ...
            val prevDeviceContentSummary = dcusEvent.prevData.getOrElse(DeviceContentSummary(dcusEvent.index.device_id, dcusEvent.index.content_id, dcusEvent.index.app_id, dcusEvent.index.channel, None, None, None, None, None, None, None, None, None, None, None, None))
