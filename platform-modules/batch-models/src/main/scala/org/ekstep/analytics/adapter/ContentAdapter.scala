    ...
    def _searchTextbook(offset: Int, limit: Int): ContentResult = {
      ...
      val body =
        s"""
                   ...
                   |}
         """.stripMargin
      var response: ContentResponse = RestUtil.post[ContentResponse](Constants.COMPOSITE_SEARCH_URL, body);
      if (null == response) {
        response = ContentResponse("", "", "", Params(None, None, None, None, None), "", ContentResult(0, Option(Array[Map[String, AnyRef]]())))
      }
      response.result;
