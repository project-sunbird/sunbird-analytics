...
case class ReducedContentDetails(content_id: String, pdata: PData, channelId: String, transfer_count: Double, did: String, origin: String, ts: Long, syncts: Long)
...
object ContentSideloadingSummaryModel extends IBatchModelTemplate[Event, ContentSideloadingInput, ContentSideloadingOutput, MeasuredEvent] with Serializable {
    ...
    override def preProcess(data: RDD[Event], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[ContentSideloadingInput] = {
        ...
        val contentMap = reducedData.groupBy { x => ContentSideloadingIndex(x.content_id, x.pdata.id, x.channelId) }.partitionBy(new HashPartitioner(JobContext.parallelization))
    ...
    override def algorithm(data: RDD[ContentSideloadingInput], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[ContentSideloadingOutput] = {
        ...
        val content_details = data.map { x => x.currentDetails }.flatMap { x => x }.map(x => (DeviceSummaryIndex(x.did, x.pdata.id, x.channelId), Buffer(x)))
        ...
        val download_details = content_details.flatMap { x => x._2 }.map { x =>
            ...
            (x.did, x.content_id, x.pdata.id, x.channelId, downloaded, download_date, DateTime.now())
        }
    ...
    override def postProcess(data: RDD[ContentSideloadingOutput], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
        ...
        data.map { csSummary =>
            ...
            MeasuredEvent("ME_CONTENT_SIDELOADING_SUMMARY", System.currentTimeMillis(), csSummary.synts, "1.0", mid, "", Option(csSummary.summary.channelId), Option(csSummary.summary.content_id), None,
