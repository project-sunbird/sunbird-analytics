    ...
    override def algorithm(data: RDD[DeviceContext], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[DeviceRecos] = {
        ...
        */
        ...
        if (config.getOrElse("saveDataFrame", false).asInstanceOf[Boolean]) {
            ...
            OutputDispatcher.dispatchDF(Dispatcher("file", Map("file" -> libfmInputFile)), rdd.rdd, columnNames);
        }
        ...
        FileDispatcher.dispatchRETrainingFile(Map("file1" -> trainDataFile, "file2" -> libfmFile), dataStr.rdd);
