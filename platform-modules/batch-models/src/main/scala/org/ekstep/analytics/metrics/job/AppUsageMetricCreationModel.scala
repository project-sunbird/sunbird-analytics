...
object AppUsageMetricCreationModel extends MetricsBatchModel[String, MeasuredEvent] with Serializable {
    ...
    def execute(events: RDD[String], jobParams: Option[Map[String, AnyRef]])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
        ...
        res.map { x =>
            x._2.map { f =>
                ...
                getMeasuredEvent(event_id, syncts, mid, f.d_channel, "AppUsageMetrics", CommonUtil.caseClassToMap(f) - ("d_period", "d_author_id", "d_app_id", "d_channel", "unique_users", "updated_date"), Dimensions(None, None, None, None, None, None, Option(PData(f.d_app_id, "1.0")), None, None, None, None, Option(f.d_period), None, None, None, None, None, None, None, None, None, Option(f.d_author_id)))            }
        }.flatMap { x => x }
    }
}