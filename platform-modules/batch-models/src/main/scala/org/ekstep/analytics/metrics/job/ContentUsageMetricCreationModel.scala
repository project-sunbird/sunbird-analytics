...
object ContentUsageMetricCreationModel extends MetricsBatchModel[String,String] with Serializable {
    ...
    def execute(events: RDD[String], jobParams: Option[Map[String, AnyRef]])(implicit sc: SparkContext) : RDD[String] ={
        ...
        val fetchDetails = ConfigDetails(Constants.CONTENT_KEY_SPACE_NAME, Constants.CONTENT_USAGE_SUMMARY_FACT, start_date, end_date, AppConf.getConfig("metrics_consumption_dataset_id") + event_id.toLowerCase() + "/", ".json", AppConf.getConfig("metrics_dispatch_to"), dispatchParams)
        ...
        val resRDD = res.mapValues { x =>
            x.map { f =>
                ...
                val event = getMeasuredEvent(event_id, mid, "ContentUsageMetrics", CommonUtil.caseClassToMapWithDateConversion(f) - ("d_period", "d_tag", "d_content_id", "m_device_ids", "updated_date"), Dimensions(None, None, None, None, None, None, None, None, None, Option(f.d_tag), Option(f.d_period), Option(f.d_content_id), None, None, None, None, None, None, None, None, None, None, None, None))
