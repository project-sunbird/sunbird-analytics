...
object AssetSnapshotMetricCreationModel extends MetricsBatchModel[String,String] with Serializable {
    ...
    def execute(events: RDD[String], jobParams: Option[Map[String, AnyRef]])(implicit sc: SparkContext) : RDD[String] ={
        ...
        val fetchDetails = ConfigDetails(Constants.CONTENT_KEY_SPACE_NAME, Constants.ASSET_SNAPSHOT_SUMMARY, start_date, end_date, AppConf.getConfig("metrics_files_prefix") + event_id.toLowerCase() + "/", ".json", AppConf.getConfig("metrics_dispatch_to"), dispatchParams)
        ...
        val resRDD = res.mapValues { x =>
            x.map { f =>
                ...
                val event = getMeasuredEvent(event_id, mid, "AssetSnapshotMetrics", CommonUtil.caseClassToMap(f) - ("d_period", "d_partner_id"), Dimensions(None, None, None, None, None, None, None, None, None, None, Option(f.d_period), None, None, None, None, None, None, None, None, None, None, Option(f.d_partner_id), None, None))
