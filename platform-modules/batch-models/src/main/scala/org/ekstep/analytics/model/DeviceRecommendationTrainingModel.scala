...
case class DeviceRecos(device_id: String, scores: List[(String, Option[Double])], updated_date: Option[DateTime] = Option(DateTime.now())) extends AlgoOutput with Output
...
object DeviceRecommendationTrainingModel extends IBatchModelTemplate[DerivedEvent, DeviceContext, Empty, Empty] with Serializable {
    ...
    val defaultDCUS = DeviceContentSummary(null, null, null, null, None, None, None, None, None, None, None, None, None, None, None, None)
    val defaultDUS = DeviceUsageSummary(null, null, null, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None)
    ...
    override def algorithm(data: RDD[DeviceContext], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[Empty] = {
        ...
        if(saveFeatureFile){
            ...
            val colBV = columns.map{x => (x, df.select(x).rdd.distinct().count())}.toMap
            val col = df.columns.map{x => (x, colBV.getOrElse(x, ""))}
            val features = sc.parallelize(col.map{x => StringUtils.remove(StringUtils.chop(x.toString()), '(')})
