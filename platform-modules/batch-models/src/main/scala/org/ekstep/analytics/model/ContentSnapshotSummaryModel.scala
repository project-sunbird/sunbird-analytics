...
object ContentSnapshotSummaryModel extends IBatchModelTemplate[DerivedEvent, DerivedEvent, ContentSnapshotAlgoOutput, MeasuredEvent] with Serializable {
    ...
    override def algorithm(data: RDD[DerivedEvent], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[ContentSnapshotAlgoOutput] = {
        ...
        val contentCountPerPartnerByStatus = GraphQueryDispatcher.dispatch(graphDBConfig, CypherQueries.CONTENT_COUNT_PER_PARTNER_BY_STATUS).list()
            .toArray().map { x => x.asInstanceOf[org.neo4j.driver.v1.Record] }
        ...
        val rdd3 = partnerUserCountRDD.leftOuterJoin(partnerActiveUserCountRDD).map { f =>
            ...
            ContentSnapshotAlgoOutput(total, live, review, f._2._1, f._2._2.getOrElse(0),"all", f._1);
        }

        // For partner_id and author_id combinations
        val contentCountPerPartnerPerAuthorByStatus = GraphQueryDispatcher.dispatch(graphDBConfig, CypherQueries.CONTENT_COUNT_PER_AUTHOR_PER_PARTNER_BY_STATUS).list()
            .toArray().map { x => x.asInstanceOf[org.neo4j.driver.v1.Record] }
        ...
        val rdd4 = sc.makeRDD(contentCountPerPartnerPerAuthorByStatus.map { f =>
            ...
            val live = f._2.filter(f => StringUtils.equals("live", f._2)).headOption.getOrElse((f._1, "live", 0L))._3;
            val review = f._2.filter(f => StringUtils.equals("review", f._2)).headOption.getOrElse((f._1, "review", 0L))._3;
            ContentSnapshotAlgoOutput(total, live, review, 0, 0, f._1._1, f._1._2);
        }.toList)
