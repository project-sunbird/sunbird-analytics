...
case class SessionSummaryInputV1(channel: String, userId: String, filteredEvents: Buffer[Event]) extends AlgoInput
case class SessionSummaryOutputV1(userId: String, appId: String, channel: String, ss: SessionSummaryV1, groupInfo: Option[(Boolean, Boolean)]) extends AlgoOutput

/**
  * Case class to hold the screener summary
  */
class SessionSummaryV1(val id: String, val ver: String, val levels: Option[Array[Map[String, Any]]], val noOfAttempts: Int, val timeSpent: Double,
...
/**
  ...
  */
object LearnerSessionSummaryV1Model extends SessionBatchModel[Event, MeasuredEvent] with IBatchModelTemplate[Event, SessionSummaryInputV1, SessionSummaryOutputV1, MeasuredEvent] with Serializable {

  implicit val className = "org.ekstep.analytics.model.LearnerSessionSummaryV1Model"

  override def name(): String = "LearnerSessionSummaryV1Model";
  ...
  override def preProcess(data: RDD[Event], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[SessionSummaryInputV1] = {
    ...
    gameSessions.map { x => SessionSummaryInputV1(x._1._1,x._1._2, x._2) }
  }

  override def algorithm(data: RDD[SessionSummaryInputV1], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[SessionSummaryOutputV1] = {
    ...
    val screenerSummary = data.map { x =>
      ...
      (LearnerProfileIndex(x.userId, pdata.id, channel), new SessionSummaryV1(gameId, gameVersion, Option(levels), noOfAttempts, timeSpent, interruptTime, timeDiff, startTimestamp, endTimestamp,
    ...
    sessionSummary.map { x => SessionSummaryOutputV1(x._1.learner_id, x._1.app_id, x._1.channel, x._2._1, x._2._2) }
  }

  override def postProcess(data: RDD[SessionSummaryOutputV1], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
