...
case class GenieSummaryV1(did: String, timeSpent: Double, time_stamp: Long, content: Buffer[String], contentCount: Int, syncts: Long,
                        etags: Option[ETags] = Option(ETags(None, None, None)), dateRange: DtRange, stageSummary: Iterable[GenieStageSummaryV1], pdata: PData, channel: String) extends AlgoOutput
case class LaunchSessionsV1(channel: String, did: String, events: Buffer[Event]) extends AlgoInput
case class GenieStageSummaryV1(stageId: String, sid: String, timeSpent: Double, visitCount: Int, interactEventsCount: Int, interactEvents: List[Map[String, String]])
case class StageDetailsV1(timeSpent: Double, interactEvents: Buffer[Event], visitCount: Int, sid: String)

object GenieLaunchSummaryV1Model extends SessionBatchModel[Event, MeasuredEvent] with IBatchModelTemplate[Event, LaunchSessionsV1, GenieSummaryV1, MeasuredEvent] with Serializable {

    val className = "org.ekstep.analytics.model.GenieLaunchSummaryModel"
    override def name: String = "GenieLaunchSummaryModel"

    def computeGenieScreenSummary(events: Buffer[Event]): Iterable[GenieStageSummaryV1] = {
        ...
        var stageMap = HashMap[String, StageDetailsV1]();
        ...
        if (screenInteractCount > 0) {
            ...
            stageList.foreach { x =>
                ...
                if (stageMap.getOrElse(currStage, null) == null) {
                    stageMap.put(currStage, StageDetailsV1(x._2, x._3, 0, x._4));
                } else {
                    stageMap.put(currStage, StageDetailsV1(stageMap.get(currStage).get.timeSpent + x._2, stageMap.get(currStage).get.interactEvents ++ x._3, stageMap.get(currStage).get.visitCount, stageMap.get(currStage).get.sid));
                }
                if (currStage.equals(x._1)) {
                    if (prevStage != currStage)
                        stageMap.put(currStage, StageDetailsV1(stageMap.get(currStage).get.timeSpent, stageMap.get(currStage).get.interactEvents, stageMap.get(currStage).get.visitCount + 1, stageMap.get(currStage).get.sid));
                    currStage = null;
                }
                prevStage = x._1;
            }
        ...
        stageMap.map { x =>
            ...
            GenieStageSummaryV1(x._1, x._2.sid, x._2.timeSpent, x._2.visitCount, x._2.interactEvents.length, interactEventsDetails)
        }
    }

    override def preProcess(data: RDD[Event], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[LaunchSessionsV1] = {
    ...
    override def algorithm(data: RDD[LaunchSessionsV1], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[GenieSummaryV1] = {
        ...
        data.map { x =>
            ...
            GenieSummaryV1(x.did, timeSpent.getOrElse(0d), endTimestamp, content, content.size, syncts, Option(CommonUtil.getETags(geEnd)), dtRange, stageSummary, pdata, channel);
        }.filter { x => (x.timeSpent >= 0) }
    }

    override def postProcess(data: RDD[GenieSummaryV1], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
