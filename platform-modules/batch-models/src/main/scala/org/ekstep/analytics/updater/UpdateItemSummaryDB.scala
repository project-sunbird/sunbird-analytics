...
case class ItemUsageSummaryFact_T(d_period: Int, d_tag: String, d_content_id: String, d_item_id: String, m_total_ts: Double, m_total_count: Int, m_correct_res_count: Int, m_inc_res_count: Int, m_correct_res: List[String], m_incorrect_res: List[InCorrectRes], m_avg_ts: Double, m_last_gen_date: Long) extends AlgoOutput
...
object UpdateItemSummaryDB extends IBatchModelTemplate[DerivedEvent, DerivedEvent, ItemUsageSummaryFact, ItemUsageSummaryIndex] with Serializable {
    ...
    private def rollup(data: RDD[ItemUsageSummaryFact_T], period: Period): RDD[ItemUsageSummaryFact] = {
        val currentData = data.map { x =>
            ...
            (ItemUsageSummaryIndex(d_period, x.d_tag, x.d_content_id, x.d_item_id), ItemUsageSummaryFact_T(d_period, x.d_tag, x.d_content_id, x.d_item_id, x.m_total_ts, x.m_total_count, x.m_correct_res_count, x.m_inc_res_count, x.m_correct_res, x.m_incorrect_res, x.m_avg_ts, x.m_last_gen_date));
        ...
        val rollupSummaries = joinedData.map { x =>
            val index = x._1
            val newSumm = x._2._1
            val prvSumm = x._2._2.getOrElse(ItemUsageSummaryFact(index.d_period, index.d_tag, index.d_content_id, index.d_item_id, 0.0, 0, 0, 0, List(), List(), List(), 0.0, List()))
    ...
    private def reduce(fact1: ItemUsageSummaryFact, fact2: ItemUsageSummaryFact_T, period: Period): ItemUsageSummaryFact = {
        ...
        val avg_ts = CommonUtil.roundDouble((total_ts / total_count), 2)
        ItemUsageSummaryFact(fact1.d_period, fact1.d_tag, fact1.d_content_id, fact1.d_item_id, total_ts, total_count, correct_res_count, inc_res_count, correct_res, incorrect_res, top5_incorrect_res, avg_ts, top5_mmc);
    }
    ...
    private def reduceItemSummary(fact1: ItemUsageSummaryFact_T, fact2: ItemUsageSummaryFact_T): ItemUsageSummaryFact_T = {
        ...
        val avg_ts = CommonUtil.roundDouble((total_ts / total_count), 2)
        ItemUsageSummaryFact_T(fact1.d_period, fact1.d_tag, fact1.d_content_id, fact1.d_item_id, total_ts, total_count, correct_res_count, inc_res_count, correct_res, incorrect_res, avg_ts, fact2.m_last_gen_date);
    }
