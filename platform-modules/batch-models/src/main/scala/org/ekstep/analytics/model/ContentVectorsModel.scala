...
object ContentVectorsModel extends IBatchModelTemplate[Empty, ContentAsString, ContentEnrichedJson, MeasuredEvent] with Serializable {
    ...
    private def getME(data: ContentEnrichedJson): MeasuredEvent = {
        ...
        MeasuredEvent("AN_ENRICHED_CONTENT", ts, ts, "1.0", mid, null, "", Option(data.contentId), None, Context(PData("AnalyticsDataPipeline", "1.0", Option("ContentToVec")), None, null, dateRange), null, MEEdata(Map("enrichedJson" -> data.jsonData)));
    }
