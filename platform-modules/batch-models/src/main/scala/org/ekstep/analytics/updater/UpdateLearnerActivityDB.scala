 ...
 * @author Santhosh
 */
object UpdateLearnerActivityDB extends IBatchModelTemplate[DerivedEvent, DerivedEvent, LearnerSnapshot, LearnerSnapshot] with Serializable {

    implicit val className = "org.ekstep.analytics.updater.UpdateLearnerActivityDB"
    override def name: String = "UpdateLearnerActivityDB"
    ...
    override def postProcess(data: RDD[LearnerSnapshot], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[LearnerSnapshot] = {
        JobLogger.log("Saving learner snapshot to DB")
