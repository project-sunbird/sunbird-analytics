...
case class DeviceUsageSummary(device_id: String, app_id: String, channel_id: String, start_time: Option[Long], end_time: Option[Long], num_days: Option[Long], total_launches: Option[Long], total_timespent: Option[Double],
...
case class DeviceUsageSummary_T(data: DeviceUsageSummary, syncts: Long, pdata: PData) extends AlgoOutput

case class DeviceUsageInput(index: DeviceSummaryIndex, currentData: Buffer[DerivedEvent], previousData: Option[DeviceUsageSummary]) extends AlgoInput
...
object DeviceUsageSummaryModel extends IBatchModelTemplate[DerivedEvent, DeviceUsageInput, DeviceUsageSummary_T, MeasuredEvent] with Serializable {
    ...
    override def preProcess(data: RDD[DerivedEvent], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[DeviceUsageInput] = {
        ...
        val newGroupedEvents = filteredEvents.map{event => 
            ...
            (DeviceSummaryIndex(event.dimensions.did.get, pdata.id, channelId), Buffer(event))
        }
            .partitionBy(new HashPartitioner(JobContext.parallelization))
            .reduceByKey((a, b) => a ++ b);
        val prevDeviceSummary = newGroupedEvents.map(f => f._1).joinWithCassandraTable[DeviceUsageSummary](Constants.DEVICE_KEY_SPACE_NAME, Constants.DEVICE_USAGE_SUMMARY_TABLE)
    ...
    override def algorithm(data: RDD[DeviceUsageInput], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[DeviceUsageSummary_T] = {
        ...
        data.map { events =>
            ...
            val pdata = CommonUtil.getAppDetails(endEvent)
            val eventsSortedByDateRange = events.currentData.sortBy { x => x.context.date_range.from };
            val prevUsageSummary = events.previousData.getOrElse(DeviceUsageSummary(events.index.device_id, events.index.app_id, events.index.channel_id, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None));
    ...
    override def postProcess(data: RDD[DeviceUsageSummary_T], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
        ...
        data.map { x =>
            ...
            val mid = CommonUtil.getMessageId("ME_DEVICE_USAGE_SUMMARY", usageSummary.device_id, "CUMULATIVE", DtRange(0l, 0l), "NA", Option(usageSummary.app_id), Option(usageSummary.channel_id));
            val measures = Map(
            ...
            MeasuredEvent("ME_DEVICE_USAGE_SUMMARY", System.currentTimeMillis(), x.syncts, "1.0", mid, "", Option(usageSummary.channel_id), None, None,
