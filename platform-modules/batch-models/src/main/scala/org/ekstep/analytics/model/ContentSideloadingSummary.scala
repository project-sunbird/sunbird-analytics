...
object ContentSideloadingSummary extends IBatchModel[Event] with Serializable {
    ...
    def execute(data: RDD[Event], jobParams: Option[Map[String, AnyRef]])(implicit sc: SparkContext): RDD[String] = {
        ...
        val contentSideloadingSummary = joinedContentData.mapValues{ x => 
            ...
            val prevContentSummary = x._2.getOrElse(ContentSideloading(contentID,0,0,0,Map(" " -> 0.0),0.0))
            ...
            val currentOriginMap = x._1.groupBy{y => y.origin}.mapValues{z => z.map{x => x.transfer_count}.max}
            val prevOriginMap = prevContentSummary.origin_map
            //Creating new origin map comparing current and previous maps
            val newOriginMap = currentOriginMap.map{ x =>
                if(prevOriginMap.get(x._1) != None) 
                    if(prevOriginMap.get(x._1).get > x._2) (x._1,prevOriginMap.get(x._1).get) 
                    else (x._1,x._2)
                else (x._1,x._2)
            }
