...
case class ContentSideloding(content_id: String, num_downloads: Long, num_devices: Long)
...
object ContentSideloadingSummary extends IBatchModel[Event] with Serializable {
    ...
    def execute(data: RDD[Event], jobParams: Option[Map[String, AnyRef]])(implicit sc: SparkContext): RDD[String] = {
        ...
        val reducedData = filteredEvents.map{ event => 
            ...
            contents.map{ f => 
                (f.getOrElse("identifier", "").asInstanceOf[String],f.getOrElse("transferCount", "").asInstanceOf[Double],event.did,CommonUtil.getEventTS(event))
            }
        }.flatMap(f=>f)
        ...
        val contentSideloadingSummary = joinedContentData.mapValues{ x => 
            ...
            val prevContentSummary = x._2.getOrElse(ContentSideloding(contentID,0,0))
            ...
            val num_devices = newContentMap.map{y => y._3}.toList.size + prevContentSummary.num_devices
            
            (ContentSideloding(contentID,num_downloads,num_devices),dtRange)
        }
