...
object LearnerActivitySummaryModel extends IBatchModelTemplate[DerivedEvent, LearnerActivityInput, TimeSummary, MeasuredEvent] with Serializable {

    implicit val className = "org.ekstep.analytics.model.LearnerActivitySummaryModel"
    override def name: String = "LearnerActivitySummaryModel"
    ...
    override def postProcess(data: RDD[TimeSummary], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
        data.map { summary =>
            ...
            MeasuredEvent("ME_LEARNER_ACTIVITY_SUMMARY", System.currentTimeMillis(), summary.dtRange.to, "1.0", mid, summary.uid, None, None, None,
