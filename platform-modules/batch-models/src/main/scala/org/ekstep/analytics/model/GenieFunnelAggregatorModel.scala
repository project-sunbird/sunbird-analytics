...
//case class FunnelSummary(funnel: String, did: String, period: Int, dspec: Map[String, AnyRef], genieVer: String, summary: Map[String, StageAggSumm], totalCount: Int, totalTimeSpent: Double, avgTimeSpent: Double, completionPercentage: Double, syncts: Long, dateRange: DtRange, tags: Option[AnyRef]) extends AlgoOutput
...
case class StageAggSumm(timeSpent: Double, totalCount: Int, avgTimespent: Double, totalInvocations: Int, completionPercentage: Double, dropoffPercentage: Double)
...
object GenieFunnelAggregatorModel extends IBatchModelTemplate[DerivedEvent, DeviceFunnelSummary, FunnelSummary, MeasuredEvent] with Serializable {
    ...
    private def _getFunnelSummary(data: DeviceFunnelSummary, stage: Stage): FunnelSummary = {
        ...
        //val contentPlayedSumm = stageSumm.get("contentPlayed").get
    ...
    private def _getStageSummMap(eksMaps: Buffer[Map[String, AnyRef]], stage: Stage): Map[String, StageAggSumm] = {
        ...
        }.toMap.map { x => (x._1, _getStageAggSumm(x._2)) }
    ...
    private def _getStageAggSumm(data: Buffer[Map[String, AnyRef]]): StageAggSumm = {
        ...
        val avgTimespent = if (0 != totalCount) CommonUtil.roundDouble(timeSpent / totalCount, 2) else 0d
        StageAggSumm(timeSpent, totalCount, avgTimespent, stageInvokedCount, CommonUtil.roundDouble(100 - dropoffPercentage, 2), dropoffPercentage);
    }
