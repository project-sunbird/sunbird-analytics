...
case class FunnelSummary(funnel: String, did: String, period: Int, dspec: Map[String, AnyRef], genieVer: String, summary: Map[String, StageAggSumm], totalTimeSpent: Double, completionPercentage: Double, syncts: Long, dateRange: DtRange, tags: Option[AnyRef]) extends AlgoOutput
...
object GenieFunnelAggregatorModel extends IBatchModelTemplate[DerivedEvent, DeviceFunnelSummary, FunnelSummary, MeasuredEvent] with Serializable {
    ...
    private def _getFunnelSummary(data: DeviceFunnelSummary, stage: Stage): FunnelSummary = {
        ...
        FunnelSummary(data.funnel, data.did, period, dimensions.dspec.getOrElse(Map()), dimensions.genieVer.get, stageSumm, totalTimeSpent, completionPercentage, firstEvent.syncts, DtRange(firstEvent.context.date_range.from, lastEvent.context.date_range.to), firstEvent.tags)
    }
