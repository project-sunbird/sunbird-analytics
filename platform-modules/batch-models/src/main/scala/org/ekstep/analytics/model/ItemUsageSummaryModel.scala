...
object ItemUsageSummaryModel extends IBatchModelTemplate[DerivedEvent, DerivedEvent, ItemSummaryOutput, MeasuredEvent] with Serializable {

    val className = "org.ekstep.analytics.model.ItemUsageSummaryModel"
    override def name(): String = "ItemUsageSummaryModel";
    ...
    override def postProcess(data: RDD[ItemSummaryOutput], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
        ...
        data.map { summary =>
            val mid = CommonUtil.getMessageId("ME_ITEM_SUMMARY", summary.itemResponse.itemId, summary.uid, summary.itemResponse.time_stamp.get);
            ...
            MeasuredEvent("ME_ITEM_SUMMARY", System.currentTimeMillis(), summary.syncts, "1.0", mid, summary.uid, Option(summary.gdata.id), None,
                Context(pdata, None, "EVENT", DtRange(summary.itemResponse.time_stamp.get, summary.itemResponse.time_stamp.get)),
