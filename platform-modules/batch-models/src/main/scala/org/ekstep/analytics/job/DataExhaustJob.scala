            ...
            for (request <- requests) {
                ...
                try {
                    ...
                    if(status.equals("COMPLETED"))
                        JobLogger.end("DataExhaust Job Completed for "+requestId, "SUCCESS", Option(Map("tag" -> clientKey, "inputEvents" -> inputEventsCount, "outputEvents" -> outputEventsCount, "timeTaken" -> exeMetrics._1, "channel" -> "in.ekstep")), "org.ekstep.analytics", requestId);
