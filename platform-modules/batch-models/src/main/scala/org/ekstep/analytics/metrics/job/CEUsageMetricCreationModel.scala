...
object CEUsageMetricCreationModel extends MetricsBatchModel[String, MeasuredEvent] with Serializable {
    ...
    def execute(events: RDD[String], jobParams: Option[Map[String, AnyRef]])(implicit sc: SparkContext) : RDD[MeasuredEvent] ={
        ...
        val groupFn = (x: CEUsageSummaryFact) => { (x.d_period + "-" + x.d_content_id + "-" + x.d_app_id + "-" + x.d_channel) };
        ...
        res.map { x =>
            x._2.map { f =>
                ...
                val mid = CommonUtil.getMessageId(event_id, f.d_content_id, "DAY", syncts, Option(f.d_app_id), Option(f.d_channel));
                getMeasuredEvent(event_id, syncts, mid, f.d_channel, "CEUsageMetrics", CommonUtil.caseClassToMap(f) - ("d_period", "d_content_id", "d_app_id", "d_channel", "updated_date"), Dimensions(None, None, None, None, None, None, Option(PData(f.d_app_id, "1.0")), None, None, None, None, Option(f.d_period), Option(f.d_content_id)))
            }
        }.flatMap { x => x }
    }
}