...
object CEUsageMetricCreationModel extends MetricsBatchModel[String,String] with Serializable {
    ...
    def execute(events: RDD[String], jobParams: Option[Map[String, AnyRef]])(implicit sc: SparkContext) : RDD[String] ={
        ...
        val resRDD = res.mapValues { x =>
            x.map { f =>
                val mid = CommonUtil.getMessageId(event_id, f.d_content_id + f.d_period, "DAY", System.currentTimeMillis(), Option(f.d_app_id), Option(f.d_channel));
                val event = getMeasuredEvent(event_id, mid, f.d_channel, "CEUsageMetrics", CommonUtil.caseClassToMap(f) - ("d_period", "d_content_id", "d_app_id", "d_channel", "updated_date"), Dimensions(None, None, None, None, None, None, Option(PData(f.d_app_id, "1.0")), None, None, None, None, Option(f.d_period), Option(f.d_content_id)))
