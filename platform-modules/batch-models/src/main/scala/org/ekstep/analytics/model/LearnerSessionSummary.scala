...
object LearnerSessionSummary extends SessionBatchModel[Event, MeasuredEvent] with IBatchModelTemplate[Event, SessionSummaryInput, SessionSummaryOutput, MeasuredEvent] with Serializable {
    ...
    override def preProcess(data: RDD[Event], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[SessionSummaryInput] = {
        JobLogger.log("Filtering Events of OE_ASSESS,OE_START, OE_END, OE_LEVEL_SET, OE_INTERACT, OE_INTERRUPT", className, None, None, None)
    ...
    override def algorithm(data: RDD[SessionSummaryInput], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[SessionSummaryOutput] = {
        ...
        JobLogger.log("Fetching the Content and Item data from Learning Platform", className, None, None, None)
        val contents = ContentAdapter.getAllContent();
        val itemData = getItemData(contents, gameList, "v2");
        JobLogger.log("Broadcasting data to all worker nodes", className, None, None, None)
        ...
        JobLogger.log("Performing Game Sessionization", className, None, None, None)
        ...
        JobLogger.log("Calculating Screen Summary", className, None, None, None)
        ...
        JobLogger.log("'screenerSummary' joining with LearnerProfile table to get group_user value for each learner", className, None, None, None)
