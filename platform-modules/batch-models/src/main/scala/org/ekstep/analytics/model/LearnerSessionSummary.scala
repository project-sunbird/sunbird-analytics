...
object LearnerSessionSummary extends SessionBatchModel[Event, MeasuredEvent] with IBatchModelTemplate[Event, SessionSummaryInput, SessionSummaryOutput, MeasuredEvent] with Serializable {
    ...
    implicit val className = "org.ekstep.analytics.model.LearnerSessionSummary"
...
//                }
                val res = telemetryVer match {
                case "2.0" =>
                    val resValues = if (null == x.edata.eks.resvalues) Array[String](); else x.edata.eks.resvalues.flatten.map { x => (x._1 + ":" + x._2.asInstanceOf[String]) };
                    resValues;
                case _ =>
                    x.edata.eks.res;
            }
