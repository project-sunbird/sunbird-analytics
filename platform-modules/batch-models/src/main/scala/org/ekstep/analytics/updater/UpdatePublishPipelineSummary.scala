...
case class PublishPipelineSummaryFact(d_period: Int, d_app_id: String, d_channel_id: String, `type`: String, state: String, subtype: String, count: Int, updated_at: Long) extends AlgoOutput
case class ContentPublishFactIndex(d_period: Int, d_app_id: String, d_channel_id: String, `type`: String, state: String, subtype: String) extends Output
...
object UpdatePublishPipelineSummary extends IBatchModelTemplate[DerivedEvent, DerivedEvent, PublishPipelineSummaryFact, ContentPublishFactIndex] with IInfluxDBUpdater with Serializable {
  ...
  private def computeForPeriod(p: Period, data: RDD[DerivedEvent])(implicit sc: SparkContext): RDD[PublishPipelineSummaryFact] = {
    ...
    joined.map { d =>
      val newFact = d._2._1
      val existingFact = d._2._2.getOrElse(PublishPipelineSummaryFact(newFact.d_period, newFact.d_app_id, newFact.d_channel_id, newFact.`type`, newFact.state, newFact.subtype, 0, DateTime.now().getMillis))
  ...
  private def createFactsFromEvent(acc: List[(ContentPublishFactIndex, PublishPipelineSummaryFact)], d: DerivedEvent)(implicit period: Period): List[(ContentPublishFactIndex, PublishPipelineSummaryFact)] = {
    ...
    val facts = pps.map { s =>
      ...
      (ContentPublishFactIndex(d_period, appId, channelId, `type`, state, subtype), PublishPipelineSummaryFact(d_period, appId, channelId, `type`, state, subtype, count, DateTime.now().getMillis))
    }
  ...
  private def combineFacts(f1: PublishPipelineSummaryFact, f2: PublishPipelineSummaryFact): PublishPipelineSummaryFact = {
    PublishPipelineSummaryFact(f1.d_period, f1.d_app_id, f1.d_channel_id, f1.`type`, f1.state, f1.subtype, f1.count + f2.count, DateTime.now().getMillis)
  }
  ...
  override def postProcess(data: RDD[PublishPipelineSummaryFact], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[ContentPublishFactIndex] = {
    ...
    saveToInfluxDB(data);
    data.map { d => ContentPublishFactIndex(d.d_period, d.d_app_id, d.d_channel_id, d.`type`, d.state, d.subtype) }
  }
  ...
  private def saveToInfluxDB(data: RDD[PublishPipelineSummaryFact])(implicit sc: SparkContext) {
		val influxRDD = data.filter(f => f.d_period != 0).map{ f =>
			...
			var tags = Map("type" -> f.`type`, "state" -> f.state, "period" -> time._2, "app_id" -> f.d_app_id, "channel_id" -> f.d_channel_id)
			if (f.subtype != "") {
