...
object DataExhaustPackager extends optional.Application {
    ...
    def packageExhaustData(jobRequest: JobRequest)(implicit sc: SparkContext) {
        ...
        try {
            ...
            JobLogger.end("DataExhaust Job Completed for "+requestId, "SUCCESS", Option(Map("client_key" -> clientKey, "inputEvents" -> inputEventsCount, "outputEvents" -> outputEventsCount, "timeTaken" -> (exhaustExeTime + packageExeTime), "SubmittedDate" -> jobRequest.dt_job_submitted.toString(), "type" -> "User")), "org.ekstep.analytics", requestId);
        } catch {
            case t: Throwable =>
                t.printStackTrace();
                ...
                JobLogger.end("DataExhaust Job Failed for "+requestId, "FAILED", Option(Map("client_key" -> clientKey, "inputEvents" -> inputEventsCount, "outputEvents" -> 0, "timeTaken" -> exhaustExeTime, "SubmittedDate" -> jobRequest.dt_job_submitted.toString(), "type" -> "User")), "org.ekstep.analytics", requestId);
        }
    }
