...
class DerivedEvent(val eid: String, val ets: Long, val syncts: Long, val ver: String, val mid: String, val uid: String, val content_id: String,
                   val context: Context, val dimensions: Dimensions, val edata: MEEdata, val etags: ETags) extends Input with AlgoInput;
...
/* Cassandra Models */
case class ContentSummaryIndex(d_period: Int, d_content_id: String, d_tag: String, d_app_id: String, d_channel_id: String) extends Output;
case class ItemUsageSummaryIndex(d_period: Int, d_tag: String, d_content_id: String, d_item_id: String, d_app_id: String, d_channel_id: String) extends Output
case class ContentUsageSummaryFact(d_period: Int, d_content_id: String, d_tag: String, d_app_id: String, d_channel_id: String, m_publish_date: DateTime, m_last_sync_date: DateTime, m_last_gen_date: DateTime, m_total_ts: Double, m_total_sessions: Long, m_avg_ts_session: Double, m_total_interactions: Long, m_avg_interactions_min: Double, m_total_devices: Long, m_avg_sess_device: Double, m_device_ids: Array[Byte], updated_date: Option[DateTime] = Option(DateTime.now())) extends AlgoOutput with CassandraTable;
...
case class ContentPopularitySummaryFact2(d_period: Int, d_content_id: String, d_tag: String, d_app_id: String, d_channel_id: String, m_downloads: Long, m_side_loads: Long, m_comments: List[(String, Long)], m_ratings: List[(Double, Long)], m_avg_rating: Double, updated_date: Option[DateTime] = Option(DateTime.now())) extends AlgoOutput with CassandraTable;
case class ItemUsageSummaryFact(d_period: Int, d_tag: String, d_content_id: String, d_item_id: String, d_app_id: String, d_channel_id: String, m_total_ts: Double, m_total_count: Int, m_correct_res_count: Int, m_inc_res_count: Int, m_correct_res: List[String], m_incorrect_res: List[(String, List[String], Int)], m_top5_incorrect_res: List[(String, List[String], Int)], m_avg_ts: Double, m_top5_mmc: List[(String, Int)], updated_date: Option[DateTime] = Option(DateTime.now())) extends AlgoOutput with CassandraTable;
