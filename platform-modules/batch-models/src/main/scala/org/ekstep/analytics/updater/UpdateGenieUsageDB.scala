...
case class GenieUsageSummaryFact(d_period: Int, d_tag: String, m_total_sessions: Long, m_total_ts: Double, m_avg_ts_session: Double, m_contents: List[String], m_total_devices: Long, m_avg_sess_device: Double, m_device_ids: Array[Byte], updated_date: Option[DateTime] = Option(DateTime.now())) extends AlgoOutput
