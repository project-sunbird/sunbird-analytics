...
case class GenieUsageSummaryFact_T(d_period: Int, d_tag: String, d_app_id: String, d_channel: String, m_total_sessions: Long, m_total_ts: Double, m_avg_ts_session: Double, m_last_gen_date: Long, m_contents: Array[String], m_device_ids: List[String]) extends AlgoOutput
case class GenieUsageSummaryFact(d_period: Int, d_tag: String, d_app_id: String, d_channel: String, m_total_sessions: Long, m_total_ts: Double, m_avg_ts_session: Double, m_contents: List[String], m_total_devices: Long, m_avg_sess_device: Double, m_device_ids: Array[Byte], updated_date: Option[DateTime] = Option(DateTime.now())) extends AlgoOutput with CassandraTable
case class GenieUsageSummaryIndex(d_period: Int, d_tag: String, d_app_id: String, d_channel: String) extends Output
...
object UpdateGenieUsageDB extends IBatchModelTemplate[DerivedEvent, DerivedEvent, GenieUsageSummaryFact, GenieUsageSummaryIndex] with Serializable {
    ...
    override def algorithm(data: RDD[DerivedEvent], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[GenieUsageSummaryFact] = {
        ...
        val genieSummary = data.map { x =>
            ...
            GenieUsageSummaryFact_T(period, tag, appId, channel, total_sessions, total_ts, avg_ts_session, x.context.date_range.to, contents.toArray, device_ids);
        }.cache();
    ...
    override def postProcess(data: RDD[GenieUsageSummaryFact], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[GenieUsageSummaryIndex] = {
        ...
        data.map { x => GenieUsageSummaryIndex(x.d_period, x.d_tag, x.d_app_id, x.d_channel) };
    }
    ...
    private def rollup(data: RDD[GenieUsageSummaryFact_T], period: Period): RDD[GenieUsageSummaryFact] = {
        ...
        val currentData = data.map { x =>
            ...
            (GenieUsageSummaryIndex(d_period, x.d_tag, x.d_app_id, x.d_channel), GenieUsageSummaryFact_T(d_period, x.d_tag, x.d_app_id, x.d_channel, x.m_total_sessions, x.m_total_ts, x.m_avg_ts_session, x.m_last_gen_date, x.m_contents, x.m_device_ids));
        }.reduceByKey(reduceGUS);
        val prvData = currentData.map { x => x._1 }.joinWithCassandraTable[GenieUsageSummaryFact](Constants.CONTENT_KEY_SPACE_NAME, Constants.GENIE_LAUNCH_SUMMARY_FACT).on(SomeColumns("d_period", "d_tag", "d_app_id", "d_channel"));
        val joinedData = currentData.leftOuterJoin(prvData)
        val rollupSummaries = joinedData.map { x =>
            val index = x._1
            val newSumm = x._2._1
            val prvSumm = x._2._2.getOrElse(GenieUsageSummaryFact(index.d_period, index.d_tag, index.d_app_id, index.d_channel, 0L, 0.0, 0.0, List(), 0L, 0.0, BloomFilterUtil.getDefaultBytes(period)));
     ...
     private def reduceGUS(fact1: GenieUsageSummaryFact_T, fact2: GenieUsageSummaryFact_T): GenieUsageSummaryFact_T = {
        ...
        GenieUsageSummaryFact_T(fact1.d_period, fact1.d_tag, fact1.d_app_id, fact1.d_channel, total_sessions, total_ts, avg_ts_session, fact2.m_last_gen_date, contents, device_ids);
    }
     ...
     private def reduce(fact1: GenieUsageSummaryFact, fact2: GenieUsageSummaryFact_T, period: Period): GenieUsageSummaryFact = {
        ...
        GenieUsageSummaryFact(fact1.d_period, fact1.d_tag, fact1.d_app_id, fact1.d_channel, total_sessions, total_ts,avg_ts_session, contents.toList, total_devices, avg_sess_device, device_ids);
    }
