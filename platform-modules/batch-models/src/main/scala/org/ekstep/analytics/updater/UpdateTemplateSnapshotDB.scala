...
object UpdateTemplateSnapshotDB extends IBatchModelTemplate[Empty, Empty, TemplateSnapshotMetrics, TemplateSnapshotMetrics] with IInfluxDBUpdater with Serializable {
    ...
    override def algorithm(data: RDD[Empty], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[TemplateSnapshotMetrics] = {
        ...
        joinedData.map { x =>
            ...
            if (prevSumm == null) {
                TemplateSnapshotMetrics(period, template_id, x._1.d_app_id, x._1.d_channel, template_name, Option(category), Option(author_id), content_count, content_count, question_count, question_count)
            } else {
                TemplateSnapshotMetrics(period, template_id, x._1.d_app_id, x._1.d_channel, template_name, Option(category), Option(author_id), content_count, prevSumm.content_count_start, question_count, prevSumm.question_count_start)
            }
        }
    }
    ...
    private def saveToInfluxDB(data: RDD[TemplateSnapshotMetrics])(implicit sc: SparkContext) {
        val metrics = data.map { x =>
            ...
            InfluxRecord(Map("period" -> time._2, "template_id" -> x.d_template_id, "app_id" -> x.d_app_id, "channel" -> x.d_channel), Map("author_id" -> x.author_id.getOrElse(""), "category" -> x.category.getOrElse(""), "template_name" -> x.template_name, "content_count" -> x.content_count.asInstanceOf[AnyRef], "content_count_start" -> x.content_count_start.asInstanceOf[AnyRef], "question_count_start" -> x.question_count_start.asInstanceOf[AnyRef], "question_count" -> x.question_count.asInstanceOf[AnyRef]), time._1);
        }
        val authors = getDenormalizedData("User", data.map { x => x.author_id.getOrElse("") })
