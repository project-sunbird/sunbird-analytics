...
case class TemplateSnapshotMetrics(d_period: Int, d_template_id: String, template_name: String, category: String, author_id: String, content_count: Long, content_count_start: Long, question_count: Long, question_count_start: Long, updated_date: Option[DateTime] = Option(DateTime.now())) extends AlgoOutput with Output
case class TemplateMetrics(d_template_id: String, template_name: String, category: String, author_id: String, content_count: Long, question_count: Long)
case class TemplateSnapshotIndex(d_period: Int, d_template_id: String)
object UpdateTemplateSnapshotDB extends IBatchModelTemplate[Empty, Empty, TemplateSnapshotMetrics, TemplateSnapshotMetrics] with IInfluxDBUpdater with Serializable {
    ...
    private def getTemplateMetrics(query: String)(implicit sc: SparkContext): List[TemplateMetrics] = {
        GraphQueryDispatcher.dispatch(query).list().map { x =>
            TemplateMetrics(x.get("template_id").asString(), x.get("name").asString(), x.get("category").asList().mkString(","), x.get("author").asString(), x.get("contentCount").asLong(), x.get("questionCount").asLong())
        }.toList
    }
    ...
    override def algorithm(data: RDD[Empty], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[TemplateSnapshotMetrics] = {
        ...
        val currentData = sc.parallelize(metrics.map { x =>
            for (p <- periodsList) yield {
                ...
                (TemplateSnapshotIndex(d_period, x.d_template_id), x);
            }
        }.flatMap(f => f))

        val prvData = currentData.map { x => x._1 }.joinWithCassandraTable[TemplateSnapshotMetrics](Constants.CREATION_METRICS_KEY_SPACE_NAME, Constants.TEMPLATE_SNAPSHOT_METRICS_TABLE).on(SomeColumns("d_period", "d_template_id"));
        ...
        joinedData.map { x =>
            ...
            if (prevSumm == null) {
                TemplateSnapshotMetrics(period, template_id, template_name, category, author_id, content_count, content_count, question_count, question_count)
            } else {
                TemplateSnapshotMetrics(period, template_id, template_name, category, author_id, content_count, prevSumm.content_count_start, question_count, prevSumm.question_count_start)
            }
        }
    }
