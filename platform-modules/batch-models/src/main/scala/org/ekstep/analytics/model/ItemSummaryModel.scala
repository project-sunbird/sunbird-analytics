...
case class ItemUsageMetricsSummary(ik: ItemKey, total_ts: Double, total_count: Int, avg_ts: Double, correct_res_count: Int, correct_res: List[String], inc_res_count: Int, top5_incorrect_res: Array[Map[String, String]], dt_range: DtRange, syncts: Long) extends AlgoOutput;
...
object ItemSummaryModel extends IBatchModelTemplate[DerivedEvent, InputEventsItemSummary, ItemUsageMetricsSummary, MeasuredEvent] with Serializable {
    ...
    private def _computeMetrics(events: Buffer[ItemUsageMetricsSummary], ik: ItemKey): ItemUsageMetricsSummary = {
        ...
        val top5_incorrect_res = events.flatMap { x => x.top5_incorrect_res }.map { y => JSONUtils.serialize(y) }.groupBy(x => x)
        .map { x => (x._2.size, x._1 ) }.toArray.sorted(Ordering.by((_: (Int, String))._1).reverse).take(5).
        map{x=>JSONUtils.deserialize[Map[String, String]](x._2)};
        
        ItemUsageMetricsSummary(ik, total_ts, total_count, avg_ts, correct_res_count, correct_res, inc_res_count, top5_incorrect_res, date_range, lastEvent.syncts);
    }
    ...
    private def _getItemUsageSummary(event: DerivedEvent, period: Int, tagId: String, content_id: String, item_id: String): ItemUsageMetricsSummary = {
        ...
        val top5_incorrect_res = if (inc_res_count == 1) eksMap.get("resValues").get.asInstanceOf[List[Map[String, String]]] else List()
        ItemUsageMetricsSummary(ik, total_ts, total_count, avg_ts, correct_res_count,correct_res, inc_res_count, top5_incorrect_res.toArray, event.context.date_range, event.syncts);
    }
