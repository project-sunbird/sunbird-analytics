...
object ItemSummaryModel extends IBatchModelTemplate[DerivedEvent, DerivedEvent, ItemSummaryOutput, MeasuredEvent] with Serializable {
        ...
        data.map { event =>
            ...
            if (null != itemResponses && itemResponses.size > 0) {
                itemResponses.map { x =>
                    ...
                    ItemSummaryOutput(event.uid, event.dimensions.group_user, event.dimensions.anonymous_user, event.mid, event.syncts, event.dimensions.gdata, event.dimensions.did, event.etags, ir, pdata, channel)
                }
            } else {
    ...
    override def postProcess(data: RDD[ItemSummaryOutput], config: Map[String, AnyRef])(implicit sc: SparkContext): RDD[MeasuredEvent] = {
        ...
        data.map { summary =>
            ...
            MeasuredEvent("ME_ITEM_SUMMARY", System.currentTimeMillis(), summary.syncts, meEventVersion, mid, summary.uid, summary.channel, Option(summary.gdata.id), None,
