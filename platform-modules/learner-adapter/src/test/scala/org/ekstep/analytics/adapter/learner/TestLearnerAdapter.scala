...
import org.ekstep.analytics.adapter.model.LearnerActivitySummary
...
class TestLearnerAdapter extends FlatSpec with Matchers with Defaults.connector.Connector {
    ...
    "LearnerAdapter" should "insert a record into leaner activity summary table" in {
        ...
        val count1 = Await.result(LearnerAdapter.LearnerActivitySummary.count(), 5.seconds).get;
        val learnerActSumm = LearnerActivitySummary(UUID.randomUUID(), 12.53, 450.34, 10.23, 430.00, Map("TOUCH" -> 12.12), Map("TOUCH" -> 50.12), 45,
        ...
        val res = Await.result(LearnerAdapter.LearnerActivitySummary.store(learnerActSumm), 5.seconds);
        val count2 = Await.result(LearnerAdapter.LearnerActivitySummary.count(), 5.seconds).get;
    ...
    it should "fetch data from learner activity summary table" in {
        ...
        val learnerActSumm = LearnerActivitySummary(uuid, 12.53, 450.34, 10.23, 430.00, Map("TOUCH" -> 13.12), Map("TOUCH" -> 60.12), 45,
            new DateTime(1451915786364L), 16, List("numeracy_369", "numeracy_377"), new DateTime(1451915786364L), new DateTime(1451915786405L),
            new DateTime(1449914267000L), new DateTime(1449914532000L));
        Await.result(LearnerAdapter.LearnerActivitySummary.store(learnerActSumm), 5.seconds);
        val res = Await.result(LearnerAdapter.LearnerActivitySummary.getById(uuid), 5.seconds).get;
